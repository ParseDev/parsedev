  ...
  class Swagger < EngineBoxcar
    ...
    CTEMPLATE = [
      syst("Study this Open API Swagger file %<swagger_url>s\n",
           "and write a Ruby Program that outputs the answer to the following questions using the appropriate API calls:\n",
           "Additional context that you might need in the Ruby program: (%<context>s)\n",
           "Use the following format:\n",
           "${{Question needing API calls and code}}\n",
           "reply only with the following format:\n",
           "```ruby\n${{Ruby code with API calls and code that returns the answer with an array of object format}}\n```\n",
           "```output\n${{Output of your code}}\n```\n\n",
           ...
           "Do not query for attributes that are not required.\n",
           "Return an array of json object with only necessary attributes.\n",
           "Make use of the rest-client gem to make your requests to the API.  return the answer."),
      user("%<question>s")
    ].freeze
